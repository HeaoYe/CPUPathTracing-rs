use crate::{
    THREAD_POOL,
    color::{ColorSpace, EncodedRgb, LinearRgb, Xyz},
    image::RgbImage,
...
pub enum PixelSample<'a> {
    Radiance(SpectrumSample, WavelengthSample),
    Rgb(EncodedRgb, &'a ColorSpace),
}

impl Pixel {
    pub fn add_sample(&mut self, sample: PixelSample) {
        let xyz_sample = match sample {
            PixelSample::Radiance(spectrum_sample, ref wavelength) => {
                Xyz::from_spectrum_sample(spectrum_sample, wavelength)
            }
            PixelSample::Rgb(encoded_rgb, color_space) => {
                color_space.xyz_from_rgb(color_space.decode(encoded_rgb))
            }
        };

        if xyz_sample.is_nan() {
            return;
        }

        self.xyz_sum += xyz_sample.as_dvec3();
        self.sample_count += 1;
    }
